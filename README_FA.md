...
برای ARM Cortex-M (M3, M4, M7) روی STM32 ساخته شده و مکانیسم‌های ایمنی متعددی را ارائه می‌دهد: بررسی پشته، محافظت MPU، تایمرها، تأیید CRC و پایش ددلاین. آنها به‌صورت پیش‌فرض غیرفعال هستند — آنها را با پرچم‌های `-D` فعال کنید. اگر محصول پزشکی یا صنعتی می‌سازید، می‌توانید انطباق IEC 62304 یا IEC 61508 را در زمان کامپایل اجرا کنید — خود کامپایلر به شما می‌گوید چیزی کم است.
...
تمام مکانیسم‌ها به‌صورت **پیش‌فرض غیرفعال** هستند (حداقل حجم). آنها را در `ZenOS_Config.hpp` با پرچم‌های `-D` فعال کنید. از `-DOS_TARGET_MEDICAL=2` یا `-DOS_TARGET_INDUSTRIAL=3` برای اجرای کامپایلی استفاده کنید.
